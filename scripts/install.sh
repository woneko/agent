...
install() {
    ...
    if [ -z "$CN" ]; then
        NZ_AGENT_URL="https://${GITHUB_URL}/woneko/agent/releases/latest/download/nezha-agent_${os}_${os_arch}.zip"
    else
        _version=$(curl -m 10 -sL "https://gitee.com/api/v5/repos/sakureu/agent/releases/latest" | awk -F '"' '{for(i=1;i<=NF;i++){if($i=="tag_name"){print $(i+2)}}}')
        NZ_AGENT_URL="https://${GITHUB_URL}/sakureu/agent/releases/download/${_version}/nezha-agent_${os}_${os_arch}.zip"
    fi
